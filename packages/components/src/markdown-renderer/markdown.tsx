      ...
      case 'echarts':
        return (
          <div style={{ minHeight: '350px', minWidth: '100%', overflowX: 'scroll' }}>
            <ErrorBoundary>
              <ReactEcharts option={chartData} style={{ minWidth: '300px' }} />
            </ErrorBoundary>
          </div>
        )
      case 'svg':
...
const Link = ({ node, children, ...props }: any) => {
  if (node.properties?.href && node.properties.href?.toString().startsWith('abbr')) {
    ...
    const handleClick = (e: React.MouseEvent) => {
      console.log('trace : click handle with Link')
    ...
    return <a {...props} onClick={handleClick} className="cursor-pointer underline !decoration-primary-700 decoration-dashed">{node.children[0] ? node.children[0]?.value : 'Download'}</a>
  }
}
